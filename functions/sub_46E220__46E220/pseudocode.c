    ...
    if ( !result )
    {
      if ( a2 )
      {
        if ( TESForm::GetEditorNameLen(a2) )
        {
          ...
          return (void *)PrintError("Unable to find ingredient (%08X) on owner object \"%s\".", *(_DWORD *)ArgList, v6);
        }
        else
        {
      ...
      {
        return (void *)PrintError("Unable to find ingredient (%08X) on unknown owner.", *(_DWORD *)ArgList);
      }
    }
  }
  return result;
