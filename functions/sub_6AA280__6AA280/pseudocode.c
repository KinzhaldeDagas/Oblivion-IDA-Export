void __thiscall sub_6AA280(int this)
{
  bool v2; // zf
  int v3; // ecx
  unsigned int v4; // edx
  unsigned int v5; // eax
  _DWORD *v6; // edi
  _DWORD *v7; // ecx
  NiTMap_Entry_TESCELL *v8; // eax
  TESObjectCELL *v9; // [esp+Ch] [ebp-Ch] BYREF
  NiTMap_Entry_TESCELL *v10; // [esp+10h] [ebp-8h] BYREF
  void *v11; // [esp+14h] [ebp-4h] BYREF

  *(_BYTE *)(this + 0xA4) = 0;
  v2 = bSoundEnabled_Audio == 0;
  v9 = 0;
  if ( !v2 )
  {
    v3 = *(_DWORD *)(this + 0x300);
    v4 = *(_DWORD *)(v3 + 4);
    v5 = 0;
    if ( v4 )
    {
      v6 = *(_DWORD **)(v3 + 8);
      v7 = v6;
      while ( !*v7 )
      {
        ++v5;
        ++v7;
        if ( v5 >= v4 )
          goto LABEL_6;
      }
      v8 = (NiTMap_Entry_TESCELL *)v6[v5];
    }
    else
    {
LABEL_6:
      v8 = 0;
    }
    v10 = v8;
    while ( v10 )
    {
      sub_452600(*(NiTMap_TESCELL **)(this + 0x300), &v10, &v11, &v9);
      sub_6B6F20((float *)v9, *(float *)&v9->members.coordOrLight.coords);
    }
  }
  SoundManager_SetMusicVolume(this, *(float *)(this + 0x2F0), 1);
}
