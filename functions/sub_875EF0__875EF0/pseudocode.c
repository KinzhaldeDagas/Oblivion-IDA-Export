void __thiscall sub_875EF0(NiTArray_NiD3DPass *this, int a2, int a3, int a4, NiRenderedTexture *a5)
{
  NiD3DPass *v6; // edi
  int v7; // ebx
  UInt32 Stage; // ebx
  int v9; // eax
  int v10; // ebx
  int v11; // ebp
  NiTexture *Texture; // ebx
  int v13; // eax
  UInt32 m_uiRefCount; // ebx
  int v15; // ebp
  UInt32 v16; // ebp
  NiRenderedTexture *v17; // eax
  int v18; // ebx
  NiRenderedTexture **v19; // ebp
  bool v20; // zf
  NiTexture *v21; // ebp
  NiRenderedTexture *v22; // ebx
  NiRenderedTexture *v23; // ecx
  UInt32 v24; // [esp+38h] [ebp+Ch]
  NiTexture *v25; // [esp+38h] [ebp+Ch]

  v6 = (NiD3DPass *)dword_B47670;
  sub_848C40(*(float **)(a4 + 0x10));
  v7 = *(_DWORD *)(a4 + 0xC);
  sub_848E50(v7);
  (*((void (__thiscall **)(NiTArray_NiD3DPass *, int, int, _DWORD))this->_vtbl + 0x2F))(this, a2, v7, 0);
  Stage = v6->Stages.data->Stage;
  v24 = Stage;
  v9 = ((int (__thiscall *)(NiRenderedTexture *, _DWORD))a5->__vftable[1].super.super.DumpAttributes)(a5, 0);
  v10 = *(_DWORD *)(Stage + 4);
  v11 = v9;
  if ( v10 != v9 )
  {
    if ( v10 )
    {
      if ( !InterlockedDecrement((volatile LONG *)(v10 + 4)) )
        (**(void (__thiscall ***)(int, int))v10)(v10, 1);
    }
    *(_DWORD *)(v24 + 4) = v11;
    if ( v11 )
      InterlockedIncrement((volatile LONG *)(v11 + 4));
  }
  Texture = v6->Stages.data->Texture;
  v25 = Texture;
  v13 = sub_848FD0(a5, 0);
  m_uiRefCount = Texture->members.super.super.m_uiRefCount;
  v15 = v13;
  if ( m_uiRefCount != v13 )
  {
    if ( m_uiRefCount )
    {
      if ( !InterlockedDecrement((volatile LONG *)(m_uiRefCount + 4)) )
        (**(void (__thiscall ***)(UInt32, int))m_uiRefCount)(m_uiRefCount, 1);
    }
    v25->members.super.super.m_uiRefCount = v15;
    if ( v15 )
      InterlockedIncrement((volatile LONG *)(v15 + 4));
  }
  v16 = v6->Stages.data[2].Stage;
  v17 = (NiRenderedTexture *)dword_B43108;
  v18 = *(_DWORD *)(v16 + 4);
  v19 = (NiRenderedTexture **)(v16 + 4);
  v20 = v18 == dword_B43108;
  a5 = (NiRenderedTexture *)dword_B43108;
  if ( !v20 )
  {
    if ( v18 )
    {
      if ( !InterlockedDecrement((volatile LONG *)(v18 + 4)) )
        (**(void (__thiscall ***)(int, int))v18)(v18, 1);
      v17 = a5;
    }
    *v19 = v17;
    if ( v17 )
      InterlockedIncrement((volatile LONG *)&v17->member);
  }
  v21 = v6->Stages.data[2].Texture;
  v22 = (NiRenderedTexture *)v21->members.super.super.m_uiRefCount;
  v20 = v22 == CanopySadowMap;
  v23 = CanopySadowMap;
  a5 = CanopySadowMap;
  if ( !v20 )
  {
    if ( v22 )
    {
      if ( !InterlockedDecrement((volatile LONG *)&v22->member) )
        v22->__vftable->super.super.super.Destructor((NiRefObject *)v22, 1);
      v23 = a5;
    }
    v21->members.super.super.m_uiRefCount = (UInt32)v23;
    if ( v23 )
      InterlockedIncrement((volatile LONG *)&v23->member);
  }
  ++v6->RefCount;
  a5 = (NiRenderedTexture *)v6;
  sub_76CE40(this + 4, *((NiD3DPass **)this + 0xE), (NiD3DPass **)&a5);
  v20 = v6->RefCount-- == 1;
  if ( v20 )
    sub_7604D0(v6);
  ++*((_DWORD *)this + 0xE);
}
