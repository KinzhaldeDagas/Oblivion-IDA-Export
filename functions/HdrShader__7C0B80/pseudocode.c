NiD3DShaderDeclaration **HdrShader()
{
  ...
  v6 = (HDRShader *)FormHeapAlloc(0x124u);
  if ( v6 )
    v7 = HDRShader::HDRShader(v6);
  else
    v7 = 0;
  v7->__vftable->super.super.Unk084((BSShader *)v7);
  ((void (__thiscall *)(NiRefObject *, bool))v7->__vftable->LoadShader)((NiRefObject *)v7, v10);
  v7->__vftable->UnkB4(v7);
  shader = (HDRShader *)v1->shader;
  if ( shader != v7 )
  {
    if ( shader )
    {
      if ( !InterlockedDecrement((volatile LONG *)&shader->member) )
        shader->__vftable->super.super.super.super.super.super.Destructor((NiRefObject *)shader, 1);
    }
