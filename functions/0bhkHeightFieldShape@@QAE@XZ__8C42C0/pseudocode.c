bhkHeightFieldShape *__thiscall bhkHeightFieldShape::bhkHeightFieldShape(bhkHeightFieldShape *this, int a2)
{
  DWORD CurrentThreadId; // eax
  bhkRefObject *v4; // eax
  bhkRefObject *v5; // esi

  EnterCriticalSection(&stru_BA7C80);
  CurrentThreadId = GetCurrentThreadId();
  ++dword_BA7CFC;
  dword_BA7CF8 = CurrentThreadId;
  v4 = (bhkRefObject *)FormHeapAlloc(0x14u);
  v5 = v4;
  if ( v4 )
  {
    bhkRefObject::bhkRefObject(v4);
    v5->__vftable = (NiObjectVtbl *)&bhkShape::`vftable';
    v5[1].__vftable = 0;
    v5[1].members.m_uiRefCount = 0;
    ++dword_BA7D70;
    v5->__vftable = (NiObjectVtbl *)&bhkHeightFieldShape::`vftable';
    ++dword_BA8400;
    v5->__vftable = (NiObjectVtbl *)&bhkPlaneShape::`vftable';
    ++dword_BA810C;
  }
  else
  {
    v5 = 0;
  }
  (*(void (__thiscall **)(bhkHeightFieldShape *, bhkRefObject *, int))(*(_DWORD *)this + 0x80))(this, v5, a2);
  if ( dword_BA7CFC-- == 1 )
    dword_BA7CF8 = 0;
  LeaveCriticalSection(&stru_BA7C80);
  return (bhkHeightFieldShape *)v5;
}
